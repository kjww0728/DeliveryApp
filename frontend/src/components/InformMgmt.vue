<template>
    ...
    <v-card style="width:450px;" outlined>
        ...
        <v-card-title v-if="value._links">
            InformMgmt # {{value._links.self.href.split("/")[value._links.self.href.split("/").length - 1]}}
        </v-card-title >
        <v-card-title v-else>
            InformMgmt
        </v-card-title >
        ...
        <v-card-text>
            ...
            <String label="DeliveryInfo" v-model="value.deliveryInfo" :editMode="editMode"/>
            <String label="MenuName" v-model="value.menuName" :editMode="editMode"/>
            <String label="Status" v-model="value.status" :editMode="editMode"/>
            <Number label="MenuQty" v-model="value.menuQty" :editMode="editMode"/>
        ...
        methods: {
            ...
            async save(){
                try {
                    ...
                    if(!this.offline) {
                        if(this.isNew) {
                            temp = await axios.post(axios.fixUrl('/informMgmts'), this.value)
                        } else {
